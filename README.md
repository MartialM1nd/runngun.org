...
- Interactive MapLibre vector map at `/map`
...
- TanStack Query
- MapLibre GL JS with CARTO vector basemaps
- Vitest and Testing Library

## Configuration

The interactive map requires a CARTO basemap API key at build time. Request a key from
[CARTO](https://carto.com/basemaps/apikey), then set:

```sh
VITE_CARTO_BASEMAP_API_KEY=your-carto-basemap-api-key
```

Vite embeds this key in the browser application so it is not a secret. Register it for the
`runngun.org` domain, do not commit the real value, and use a dedicated key for this site.
...
1. Run `VITE_CARTO_BASEMAP_API_KEY='your-key' npm run build`.
